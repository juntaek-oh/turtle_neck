"""
OpenPose 결과를 디코딩하는 모듈
"""
import numpy as np


class OpenPoseDecoder:
    """OpenPose 모델의 출력을 디코딩하여 자세 정보를 추출하는 클래스"""
    ...
    def refine(heatmap, x, y):
        """키포인트 위치 정밀화"""
        h, w = heatmap.shape[-2:]
        valid = np.logical_and(np.logical_and(x > 0, x < w - 1), 
    ...
    def group_keypoints(self, all_keypoints_by_type, pafs, pose_entry_size=20):
        ...
        for part_id, paf_channel in enumerate(self.paf_indices):
            ...
            pose_entries = self.update_poses(
                kpt_a_id, kpt_b_id, all_keypoints, connections, pose_entries, pose_entry_size
            )
